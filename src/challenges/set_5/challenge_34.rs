#[cfg(test)]
mod tests {
    use crate::{cbc::*, dh::DiffieHellman, digest::sha1::SHA1, digest::Digest, util};
    use num_bigint::*;
    ...
    fn challenge_34b() {
        ...
        assert_eq!(
            std::iter::repeat(0u8)
                .take(util::get_bytes(&alice_session).len())
                .collect::<Vec<u8>>(),
            util::get_bytes(&alice_session),
        );
        ...
        assert_eq!(
            std::iter::repeat(0u8)
                .take(util::get_bytes(&bob_session).len())
                .collect::<Vec<u8>>(),
            util::get_bytes(&bob_session),
        );
